...
The simple-to-advanced progression is an interface promise, not an obligation to bundle every engine or service in the first release. The first user-testable slice is read-only inspection plus explicit setup guidance; consented provisioning and advanced execution are separate follow-on slices inside Issue #109, each verified before the next is described as delivered. No previously created consumer repository is rewritten by default.
