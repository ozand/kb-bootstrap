...
class LessonIdentityResearchTests(unittest.TestCase):
    ...
    def test_evidence_limitations_and_rename_handling_are_explicit(self):
        ...
        self.assertIn("Current implementation provides only partial provenance", text)
        self.assertIn("configured store name", text)
        self.assertIn("does not carry or validate the lesson scope", text)
        self.assertIn("not sufficient by itself", text)
